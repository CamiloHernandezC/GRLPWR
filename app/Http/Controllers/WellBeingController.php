...
class WellBeingController extends controller {
    ...
    public function processWellBeingTest(Request $request, User $user): \Illuminate\Http\RedirectResponse
    {
        DB::transaction(function () use ($request, $user) {
            ...
            $wellOfLife = new WheelOfLife();
            $wellOfLife->user_id = $user->id;
            $wellOfLife->health = $request->health;
            $wellOfLife->reason_health = $request->reason_health;
            $wellOfLife->personal_growth = $request->personal_growth;
            $wellOfLife->reason_personal_growth = $request->reason_personal_growth;
            $wellOfLife->home = $request->home;
            $wellOfLife->reason_home = $request->reason_home;
            $wellOfLife->family_and_friends = $request->family_and_friends;
            $wellOfLife->reason_family_and_friends = $request->reason_family_and_friends;
            $wellOfLife->love = $request->love;
            $wellOfLife->reason_love = $request->reason_love;
            $wellOfLife->leisure = $request->leisure;
            $wellOfLife->reason_leisure = $request->reason_leisure;
            $wellOfLife->work = $request->work;
            $wellOfLife->reason_work = $request->reason_work;
            $wellOfLife->money = $request->money;
            $wellOfLife->reason_money = $request->reason_money;
            $wellOfLife->user_id = auth()->user()->id;
            $wellOfLife->save();
        });
